...
const UserAccountRow = ({ item, onChangeStatus }) => {
    ...
    return (
        ...
        >
            ...
            <td class='col-span-2'><img class='w-[60px] h-[60px]' src={item.avatar}></img></td>
            <td class='col-span-2 break-words'>{item.email}</td>
            {/* <td class='col-span-1'>Hiển thị</td> */}
            <td class='col-span-2'>

                <select class='bg-[#e1e1e1]' style={{ background: item.status === 0 ? "#75718a" : "" }} onChange={(e) => onChangeStatus(item.username, Number(e.target.value))}>
