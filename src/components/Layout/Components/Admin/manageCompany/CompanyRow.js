    ...
    return (
        ...
        >
            ...
            <td class='col-span-2'>
                ...
                <select 
                    className={`rounded-lg p-[5px] ${item.status === 0 ? 'bg-danger' : item.status === 1 ? 'bg-success' : item.status === 2 ? "bg-warning" : ""}`}  style={{ background: item.status === 0 ? "#75718a" : "" }}
                    ...
                    <option className="bg-warning" selected={item.status === 4 ? true : false} value={4} >Khóa</option>
